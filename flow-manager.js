...
module.exports = async function(RED) {
    ...
    const isProjectModeEnabled = (()=>{
        try {
            return RED.settings.editorTheme.projects.enabled;
        } catch (e) {}
        return false;
    })();
    ...
    async function load() {
        ...
        async function refreshDirectories() {
            let basePath;
            if(isProjectModeEnabled) {
                const activeProject = await readActiveProject();
                if(activeProject == null) throw 'Project mode and no project set';
        ...
        async function loadFlows(flowsToShow = null) {
            ...
            try {
                const configNodes = JSON.parse(
                    await fsPromise.readFile(directories.configNodesFilePath)
                );
        ...
        async function eraseMainFlowsAndLoadActualFlows() {
